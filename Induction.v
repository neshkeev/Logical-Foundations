...
Theorem plus_n_Sm : forall n m : nat,
  Datatypes.S (n + m) = n + (Datatypes.S m).
Proof.
  ...
  - (* n = S n' *) induction m as [|m' IHm'].
    * (* m = 0 *) simpl. rewrite -> IHn'. reflexivity.
    * (* m = S m' *) simpl. rewrite -> IHn'. reflexivity.
Qed.

Lemma plus_n_Sm_equals_plus_Sn_m: forall n m : nat,
    n + Datatypes.S m = Datatypes.S n + m.
Proof.
  intros n m. induction n as [|n' IHn'].
  - (* n = 0 *) reflexivity.
  - (* n = S n'*) simpl. rewrite <- plus_n_Sm. reflexivity.
Qed.
...
Proof.
  ...
  - (* n = S n' *) induction m as [|m' IHm'].
    * (* m = 0 *) rewrite <- plus_n_O. simpl. reflexivity.
    * (* m = S m' *) simpl. rewrite -> IHn'. rewrite <- plus_n_Sm_equals_plus_Sn_m. reflexivity.
Qed.
...
Fixpoint double (n:nat) :=
  match n with
  | O => O
  | Datatypes.S n' => Datatypes.S (Datatypes.S (double n'))
  end.
...
Lemma mult_dist : forall n m p : nat,
    (n + m) * p = n * p + m * p.
Proof.
...
Proof.
  intros n m p. induction n as [|n' IHn'].
  - reflexivity.
  - simpl. rewrite -> IHn'. rewrite -> mult_dist. reflexivity.
Qed.
