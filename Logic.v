...
Proof.
  ...
  split.
  - intros [H1 | [H2 H3]].
    + (* H1 = P *) apply and_intro.
      * (* P v Q *) left. apply H1.
      * (* P v R *) left. apply H1.
    + (* Q ^ R *) apply and_intro.
      * (* P v Q *) right. apply H2.
      * (* P v R *) right. apply H3.
  - (*(P v Q) ^ (P v R) -> P v Q ^ R *) intros [[H1 | H2] [H3 | H4]].
    + left. apply H1.
    + left. apply H1.
    + left. apply H3.
...
Proof.
  ...
  - (* -> *)  intro H.
    ...
    + (* l = n :: l' *) simpl in H.
      ...
      * (* f n = y *) exists n. apply and_intro.
        { apply Hfy. }
        { simpl. left. reflexivity. }
      * (* f n != n *) apply IHl in Hiyl.
...
Proof.
  ...
  - destruct (evenb n') eqn:H1 in IHn.
    ...
    + rewrite -> (evenb_S n'). rewrite -> H1. simpl.
      inversion IHn.
      exists (S x). simpl. apply f_equal. apply H.
Qed.
(** [] *)
...
Proof.
  ...
  split.
  - intros H1 H2. apply beq_nat_true_iff in H2. rewrite H2 in H1. inversion H1.
  - intro H. induction x as [|x' IHn].
    ...
    + destruct y as [|y'].
      * reflexivity.
      * simpl. destruct (beq_nat x' y') eqn:H1.
        { exfalso. apply H. apply f_equal. apply beq_nat_true_iff. apply H1. }
        { reflexivity. }
Qed.
(** [] *)
...
Fixpoint beq_list {A : Type} (beq : A -> A -> bool)
  ...
  | [], _ | _, [] => false
  | h1 :: t1, h2 :: t2 =>
      if beq h1 h2 then beq_list beq t1 t2
      else false
  end.
...
Proof.
  ...
  - (* -> *) generalize dependent l2.
    ...
    + intros l2. induction l2 as [|m l2' IHl2].
      * intro H1. inversion H1.
      * simpl. intro H1.
        destruct (beq n m) eqn:He1.
        apply H in He1.
        rewrite <- He1.
        assert (H2 : (l1' = l2' -> (n :: l1' = n :: l2'))).
  ...
  - (* <- *) intro H1.
    generalize dependent l2.
    induction l1 as [|n l1' IHl].
    + (* l1 = nil *) intros l2 H1. rewrite <- H1. reflexivity.
    + (* l1 = n :: l1' *) intros l2 H1.
      destruct l2 as [|m l2'].
      * inversion H1.
      * symmetry in H1. inversion H1. 
        simpl. apply H in H2. inversion H1. rewrite -> H4 in H2.
  ...
  - (* -> *) induction l as [|n l' IHl].
    ...
    + simpl. intro H. split.
      * rewrite <- andb_commutative in H. apply andb_true_elim2 in H.
        apply H.
      * apply IHl. apply andb_true_elim2 in H.
        apply H.
  - (* <- *) induction l as [|n l' IHl].
    + reflexivity.
