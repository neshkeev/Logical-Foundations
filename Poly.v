...
Check @combine.
Compute (combine [1;2] [false;false]).
...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter (fun x => negb (leb x 7)) (filter evenb l).
...
(** Definition hd { X : Type } (l : list X): option X :=
  match l with
  | [] => None
  | h :: t => Some h
  end. *)

(** Lemma map_f_l_eq_cons_f_hd_map_f_tl : forall (X Y : Type) (f : X -> Y) (l : list X),
    map f l = cons f (hd l) nil. *)

Lemma map_f_app_l1_l2_eq_app_map_f_l1_map_f_l2 :
  forall (X Y : Type)
    (f : X -> Y)
    (l1 l2 : list X),
...
Proof.
  intros X Y. intros f l.
  induction l.
  - reflexivity.
  - simpl. rewrite <- IHl.
    rewrite -> map_f_app_l1_l2_eq_app_map_f_l1_map_f_l2.
    reflexivity.
Qed.
...
Definition fold_map {X Y:Type}
           (f : X -> Y)
           (l : list X) : list Y :=
  fold (fun (x : X) (acc : list Y) => f x :: acc) l [].
...
(* FILL IN HERE *)
 *)
(* Theorem out_of_bound: forall (X : Type) (l : list X) (n : nat),
