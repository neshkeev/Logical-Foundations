...
Proof.
  1. Induction by n:
  1.1. n = 0 : forall m : nat => beq_nat 0 m = true -> n = m.
  1.1.1 Case-analysis by m:
  1.1.1.1. m = 0 :
    beq_nat 0 0 = true -> 0 = 0 (O constructor is injective). Done.
  1.1.1.2. m = S m', beq_nat 0 m' = true :
    beq_nat 0 (S m') = true -> 0 = S m'
    false = true -> 0 = S m' (ex-falso). Done.
  1.2. n = S n', forall m : nat => beq_nat n' m = true -> n = m.
  1.2.1 Case-analysis by m:
  1.2.1.1. m = 0 :
    beq_nat (S n') 0 = true -> S n' = 0
    false = true -> S n' = 0 (ex-falso). Done
  1.2.1.2. m = S m':
    beq_nat (S n') (S m') = true -> S n' = S m' (* simpl + f_equal *)
    beq_nat n' m' = true -> n' = m' (induction hypothesys) Done.
*)
...
Proof.
  intros n m p H1 H2.
  apply eq_trans with (beq_nat n m).
  apply f_equal.
...
Proof.
  ...
  induction l as [|n l' IHl].
  - (* l = nil *) intros x lf H. simpl in H. inversion H.
  - (* l = n :: l' *) intros x lf H. simpl in H.
    destruct (test n) eqn:Htst1.
    + (* test n = true *) inversion H. rewrite H1 in Htst1. apply Htst1.
    + (* test n = true *) apply IHl in H. apply H.
Qed.
(** [] *)
...
Proof.
  ...
  - (* l = n :: l' *)
    ...
    + (* f n = true *) unfold existsb'.
      simpl.
      rewrite -> Hfn1. reflexivity.
    + (* f n = false *)
      assert (H : existsb' f (n :: l') = f n || existsb' f l').
      { rewrite -> Hfn1. unfold existsb'.
        simpl. rewrite -> Hfn1. reflexivity. }
      rewrite -> H.
      rewrite <- IHl.
      reflexivity.
Qed.
